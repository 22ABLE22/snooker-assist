...
### 2j. ~~黑球位置偏移~~ ✅ 已加固
- 原因：`V<55` 会把球体阴影、库影、袋口洞一起圈进连通域，质心偏右下
- 修复：黑球用**最暗核 + 距离变换**取内切圆心；贴库暗条、袋口邻域直接丢弃
- 若仍偏：可再调 `BALL_CENTER_BIAS_*` 或截图对比

### 2k. ~~仅支持绿色台泥~~ ✅ 已完成（多色 auto）
- `FELT_CLOTH_MODE="auto"`：按大矩形评分在 green/blue/red/purple/yellow 中选主色
- 可写死：`FELT_CLOTH_MODE="blue"` 等；HSV 见 `FELT_CLOTH_HSV`
- **限制**：红色台泥与红球同色，红球识别会明显变差（需后续用轮廓/高光加强）
- 状态栏显示 `台泥=green` 等

### 2l. ~~浅绿台泥外框偏大~~ ✅ 已完成
- 原因：库边装饰/菱形与台泥同色，外轮廓撑到库外
- 修复：**六袋黑洞定位**四角矩形 → 按袋半径内缩 → **只向内**按台泥连续性收边
- 用 `output/lightgreen_felt.png` 验证：青框贴台泥，检出白球/彩球/红球
- 注：旧黄框是历史截图上已画死的，以新标注为准

### 2m. ~~自动台面仍不准~~ ✅ 已完成（手动校准）
- F7 进入校准：拖四边，或底部「左/右/上/下」按钮微调
- 再按 F7 确认 → **锁定**；F8/F9 不再改台面大小（一盘内不变）
- F6 清除锁定，恢复自动
- 六袋按最终矩形生成；圆半径 `POCKET_DRAW_RADIUS_RATIO`（config）
- 锁定写入 `output/felt_lock.json`（相对窗口比例，窗口移动仍可用）
