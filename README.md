...
|----|------|
| **F6** | 清除台面锁定，恢复自动检测 |
| **F7** | 校准台面：拖四边或点「左/右/上/下」微调，再按 F7 锁定 |
| **F8** | 截图分析（优先进球，其次解球） |
| **F9** | 只分析进球 |
...
默认**鼠标穿透**，可直接在游戏内击球。启动脚本见 `start_overlay.bat`（热键说明为英文，避免 cmd 中文编码问题）。
...
9. **台面锁定**：F7 校准并确认后，本盘内 F8/F9 **不再改台面大小**；六袋按最终矩形生成；袋口圆半径见 `POCKET_DRAW_RADIUS_RATIO`  
